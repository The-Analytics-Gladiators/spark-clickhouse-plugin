...
class ClickhouseFieldsTest extends AnyFlatSpec with Matchers {
  "ClickhouseSchemaParser" should "parse schema with Nullables, Arrays and LowCardinalities" in {
    withTable(Seq(
      ...
      val fields  = TableInfoService.fetchFields(s"jdbc:clickhouse://$shard1Replica1", "default.test_table", new Properties()).get
