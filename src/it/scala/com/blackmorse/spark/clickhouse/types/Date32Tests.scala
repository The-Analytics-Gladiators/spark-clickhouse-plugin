...
import com.blackmorse.spark.clickhouse.sql.types.primitives.ClickhouseDate32
...
//  private val minDate = localDateToDate(LocalDate.of(1925, 1, 1)) //TODO Check with string type
  ...
  "Date32" should "be supported" in {
    ...
    testPrimitiveAndArray(new ClickhouseDate32(nullable = false, lowCardinality = false))(
      cases = Seq(
        (1 to 100).map(date),
        Seq(maxDate)
      ),
