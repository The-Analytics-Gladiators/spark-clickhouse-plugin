...
import com.blackmorse.spark.clickhouse.sql.types.extractors.{BooleanRSExtractor, SimpleArrayRSExtractor, StandardInternalRowConverter, StandardRowArrayConverter}
...
case class ClickhouseBoolean(nullable: Boolean, lowCardinality: Boolean)
    ...
    with StandardInternalRowConverter[Boolean]
    with StandardRowArrayConverter[Boolean] {
